...
class SimpleRasterWriter(object):
    ...
    def __init__(self, xll, yll, nr, nc, drc, nodata=None):
        self.xll = xll
        self.yll = yll
        self.drc = drc
        self.nr = nr
    ...
    def cell_values(self, idx, data_names=('luc',)):
    ...
    def set_cell_value(self, idx, value, data_name='luc'):
        """
...
# RS: Definition of cell centre for every cell in x and y coordinates
    ...
    def open(self, filename, input_layers):
        ...
        self.prm.write('# Generated at {dt_string_1} '.format(dt_string_1=dt_string))
        self.prm.write('from a temporary layer\n')
        self.prm.write('#  based on Land-Use-Category raster (LUC) {}  \n'.format(input_layers['luc']['layer'].name()))

        self.prm.write('# Comments are marked with #\n')
        self.prm.write('#\n')
        self.prm.write('# Explanation of data (in one line):\n')
        self.prm.write('#  !name           : User defined file name\n')
        ...
        self.prm.write('#   \t!name        = add\n')
        self.prm.write('#   \t!type        = addDropDown\n')
    ...
    def write_cell(self, data):
        ...
        for key, value in list(self.nodata.items()):
            ...
            if (self.index+1) % self.nc == 0:  # jump to next row once every value per column is written
                self.prm.write('{luc:f}\n'.format(**data))
            else:
                self.prm.write('{luc:f}\t'.format(**data))
